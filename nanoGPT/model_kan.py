...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        assert config.n_embd % config.n_head == 0, f"embedding dimension must be divisible by number of heads, embedding: {config.n_embd}, heads: {config.n_head}"
...
class Block(nn.Module):
    ...
    def __init__(self, config):
        ...
        # self.mlp = MLP(config)
        self.mlp = KAN(in_features=config.n_embd, 
                       hidden_features=config.n_embd * config.mult, 
